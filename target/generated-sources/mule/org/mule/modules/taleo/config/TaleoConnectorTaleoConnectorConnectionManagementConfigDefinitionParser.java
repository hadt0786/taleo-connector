...
import org.mule.modules.taleo.connectivity.TaleoConnectorConfigConnectionManagementConnectionManager;
...
public class TaleoConnectorTaleoConnectorConnectionManagementConfigDefinitionParser
    extends AbstractDevkitBasedDefinitionParser
{

    private static Logger logger = LoggerFactory.getLogger(TaleoConnectorTaleoConnectorConnectionManagementConfigDefinitionParser.class);

    public BeanDefinition parse(Element element, ParserContext parserContext) {
        parseConfigName(element);
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(parserContext);
        builder.setScope(BeanDefinition.SCOPE_SINGLETON);
        setInitMethodIfNeeded(builder, TaleoConnectorConfigConnectionManagementConnectionManager.class);
        setDestroyMethodIfNeeded(builder, TaleoConnectorConfigConnectionManagementConnectionManager.class);
        parseProperty(builder, element, "companyCode", "companyCode");
        parseProperty(builder, element, "dispatcherUrl", "dispatcherUrl");
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(TaleoConnectorConfigConnectionManagementConnectionManager.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the configuration [config] within the connector [taleo] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the configuration [config] within the connector [taleo] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }
